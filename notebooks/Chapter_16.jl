...
# ╔═╡ 69a39a58-a692-473b-a912-ad9478bf92ed
begin
  using Pkg, DrWatson
  using PlutoUI
  TableOfContents()
...
# ╔═╡ 192e951b-42d1-43b9-b30b-69baddb7e3f0
begin
	using Turing
	using Turing
	using DataFrames
...
# ╔═╡ e80dfdcc-1857-4fac-88dd-9217ddffb6ad
using DifferentialEquations

# ╔═╡ 029ca676-bde5-11ef-1084-ef6ca05d8846
md"# Chap 16: Generalized Linear Madness"

# ╔═╡ 5c924491-b60a-4137-908f-33dce7cd43be
versioninfo()

# ╔═╡ 50096d54-971c-4289-b04a-c44bf1cada68
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ bc598fcf-8502-41bc-87eb-3fbe34beae60
md"# 16.1 Geometric people"

# ╔═╡ f3b00a2f-4927-4437-a8d8-0e8590562f3e
md"## Code 16.1 Load data"

# ╔═╡ a25a0d65-b4f3-41b8-b737-9689dbf13843
begin
...
# ╔═╡ 5ed95489-469d-4092-bcc6-b5832ce29dda
first(d, 3)

# ╔═╡ 4cf30f87-1ca6-4f48-beec-ce74d408880f
md"## Code 16.2 `m16_1`"

# ╔═╡ 24e6e37b-e1f9-4750-b2b7-24d94ce4c322
...
# ╔═╡ 0e7466e4-f936-4817-85ff-d497903d2880
begin
	Random.seed!(1)
	@time m16_1_ch = sample(m16_1(d.w, d.h), NUTS(), 1000)
	m16_1_df = DataFrame(m16_1_ch);
end

# ╔═╡ f43460d3-f6a8-46a1-abb5-1ad5406ef94d
describe(m16_1_df)

# ╔═╡ 74f04b6e-e960-4c7b-a667-c0af5644ff94
md"## Code 16.3 Check the estimates"

# ╔═╡ c07f64df-9ede-4d45-9303-96a6c126a808
let
...
# ╔═╡ 9acf6027-5829-4d4b-8431-c4e4ed2eed36
scatter(m16_1_df.k, m16_1_df.p, xlab="K (ratio of weight/height)",
	ylab="P (Ratio of radius/height)")

# ╔═╡ a96174fb-df77-44e0-927f-7a94c3b9ec07
@df m16_1_df corrplot(cols(1:3), seriestype=:scatter, ms=0.2, size=(950, 800), bins=30, grid=false)

# ╔═╡ 2a0562f2-e6c0-4e51-afe2-2af8df83ba4c
md"# 16.2 Hidden minds and observed behavior"

# ╔═╡ b98db665-f707-427e-8547-368d588d8468
md"## Code 16.4 Load Data
- y: takes value 1, 2, 3, indicating which the three options were chosens.
  - 1: unchosen color.
...
# ╔═╡ 55d2bce3-97db-4c57-8f94-45f50a334419
begin
	d_box = DataFrame(CSV.File("data/Boxes.csv"))
	first(d_box, 3)
end

# ╔═╡ 268457cd-556b-4967-9aa4-1e8f0c597b08
size(d_box)

# ╔═╡ 162c7b13-71d4-4b20-92c7-ca41f885b784
describe(d_box)

# ╔═╡ 6d370671-c3a3-4b93-a3f3-7a5712e5e56a
md"## Code 16.5 Fraction of different outcomes"

# ╔═╡ 7ecde894-2dd3-45b1-8dc0-9bc13a5b7498
begin
	y_dict = Dict(
	    k => v / nrow(d_box)
	    for (k, v) in countmap(d_box.y)
	)
	y_dict = sort(y_dict)
end

# ╔═╡ dece429d-2206-49ea-8f41-6beddf22fe1d
md"## Code 16.6: A Simulated Strategy: Half kids choose at random and the other half follow the majority"

# ╔═╡ fe2883c8-4abc-4eac-8576-a15c743a2818
let
	Random.seed!(3)
	# number of children
	N = 1000
...
# ╔═╡ 8d6c6d5b-296f-43f8-b151-fccd7acf4a0a
100>>1

# ╔═╡ 2c25eec8-6d01-4a98-95d2-f8d0724c833a
bitstring(Int8(100))

# ╔═╡ ddcf599d-ea72-4b7c-9843-f9694929e5a6
99>>1

# ╔═╡ f296704b-a1e0-4569-a6c4-8517b5728b7a
md"## Code 16.7 `m16_2` "

# ╔═╡ 18f00edf-4f80-4a0e-a044-d4d80a7259e8
@model function m16_2(N, y, majority_first_vec)
    p ~ Dirichlet(5, 4) #5 modes (majority, minority, maverick, random, follow first). 4 is Dirichlet α.

	# a 5-element vector. Each element is a probability vector of length N (number of individuals).
    phi = [
        y .== 2,  # majority. prob vector if everyone uses the majority strategy.
        y .== 3,  # minority
        y .== 1,  # maverick
        fill(1/3, N),  # random
        [ 
            mf ? a : b 
            for (mf, a, b) ∈ zip(majority_first_vec .> 0, y .== 2, y .== 3)
        ] # follow first
    ]
    lphi = [
        log(p_v) .+ log.(phi_v)
...
# ╔═╡ 8a39bbb5-4ae9-4002-800a-cb2d3c7b94ac
md"## Code 16.8 Plot confidence interval of posterior p"

# ╔═╡ 255d3810-7f10-4614-9fd9-4b7629af2d1a
begin
	Random.seed!(1)
	@time m16_2_ch = sample(
		m16_2(nrow(d_box), d_box.y, d_box.majority_first), 
		NUTS(200, 0.65, init_ϵ=0.5), 
		1000)
	m16_2_df = DataFrame(m16_2_ch);
	
	coeftab_plot(m16_2_df, pars_names=("1 Majority", "2 Minority", 
		"3 Maverick", "4 Random", "5 Follow First"))
end

# ╔═╡ 49cf1219-d0ea-4915-8ec3-19677f7bee06
m16_2_df

# ╔═╡ 2279dac5-4506-4000-ba3d-ef21c24d6ad0
describe(m16_2_df)

# ╔═╡ 0dcf2452-0978-4596-822f-dc1ca7c558af
md"# 16.3 Ordinary differential nut cracking"

# ╔═╡ 1f59bcf6-c08e-4918-9b5e-6b3e6f111055
md"## Code 16.9 Load panda nuts data"

# ╔═╡ 7b5bf88c-bd98-4e11-a218-e19a1ce59521
begin
d_nuts = DataFrame(CSV.File("data/Panda_nuts.csv"));
first(d_nuts, 3)
end

# ╔═╡ bdf59571-5bf4-4139-bbe0-1d815782f67e
size(d_nuts)

# ╔═╡ df28440a-d971-43ff-8f55-8a7eb5a7e1c7
describe(d_nuts)
...
# ╔═╡ 79832ee3-e410-4568-941e-30f3d626b646
max_age = maximum(d_nuts.age)

# ╔═╡ 6c8afbf6-bb21-4c53-a62d-dcbd7e1752f9
let
	N = 10_000
	...
	p1 = plot(xlim=(0, 1.5*max_age), ylim=(0, 1), 
		xlab="age", ylab="body mass")
	p2 = plot(xlim=(0, 1.5*max_age), ylim=(0, 1.2), 
...
# ╔═╡ f318884d-ed4c-4109-a72b-4063f15141fe


# ╔═╡ d089d3bf-5b1a-489a-9301-facfc70f1ea8
md"## Code 16.11 `m16_4`"

# ╔═╡ fb9b88f5-4bd0-4bb8-9db0-bebdeacabbfa
begin
	dat_list = (
    n = d_nuts.nuts_opened,
    age = d_nuts.age ./ max_age,
    seconds = d_nuts.seconds,
)
end


# ╔═╡ a7061025-dd0b-437d-a402-75193ab916e2
...
# ╔═╡ 9852f360-89ed-4f33-85e8-a9943698c8a0
begin
	Random.seed!(3)
	@time m16_4_ch = sample(m16_4(dat_list...), NUTS(100, 0.65, init_ϵ=0.025), 4000)
	m16_4_df = DataFrame(m16_4_ch);
end

# ╔═╡ d4c8f261-16e1-4c83-b9af-080dcf009d30
describe(m16_4_df)

# ╔═╡ 0ff25a11-ad12-4973-b7f5-19759e3c1f09
md"## Code 16.12 Posterior estimates"

# ╔═╡ d9da76d2-22b0-486b-93d4-feb1b6ff45ee
let
	p = plot(xlim=(0, max_age), ylim=(-0.1, 1.5), 
		xlab="age", ylab="nuts per second")
...
# ╔═╡ a037e76b-9b6d-4c60-b26a-ee6deb4b7d1b
md"# 16.4 Population dynamics"

# ╔═╡ 7e9346e3-5919-4be8-a6cc-08b7600e11df
md"## Code 16.13 Load data"

# ╔═╡ 6b266054-9f74-47ea-8890-547e6f8d88c1
begin
	d_hare = DataFrame(CSV.File("data/Lynx_Hare.csv"))
...
# ╔═╡ 25af7393-b607-47a6-b214-82a43bcc3d78
describe(d_hare)

# ╔═╡ 2f9b8ebb-1ad7-4b34-981c-0f9c934cc708
md"## Code 16.14 Simulate Lynx & Hare Population"

# ╔═╡ b2bbdc9c-83e3-453b-ba61-8a7e68ccc4d9
...
# ╔═╡ c75139b9-2291-4b16-a6b5-15256381c4c3
md"## Code 16.15 Plot one simulation"

# ╔═╡ edf093ff-785c-4b5d-932e-aeb12039122c
let
	θ = [0.5, 0.05, 0.025, 0.5]
	z = sim_lynx_hare(10_000, [d_hare.Lynx[1], d_hare.Hare[1]], θ);

	plot(z[1], lw=2, xlab="time", ylab="number (thousands)", c=:blue)
	plot!(z[2], lw=2, c=:black)
end

# ╔═╡ ff3dee2a-4bbb-4d48-803f-97596c353c94
md"## Code 16.16 Simulate the number of pelts given trapping rates"
...
# ╔═╡ 8a50cc4f-4cb9-40b8-8365-a64413644b9c
md"## Code 16.17 `lynx_hare_model`"

# ╔═╡ f31b6bea-1279-4822-8823-91ea38367850
...
let
	...
	for r in first(eachrow(m16_5_df), 21)
	    L, H = row_to_population(r, nrow(d_hare))
	    L .*= r.p_l
	    H .*= r.p_h
	    
	    plot!(d_hare.Year, L, c=:blue, alpha=0.2)
	    plot!(d_hare.Year, H, c=:black, alpha=0.2)
	end
	p
end

# ╔═╡ a8c68448-219d-4505-a11c-6ae1682d5b01
md"## Code 16.20 Plot the posterior estimates of animals"

# ╔═╡ 7cbb5a61-980f-4ba7-8957-4c190d4e6565
let
...
manifest_format = "2.0"
project_hash = "75d8fff9f0abebab04146cb555080007a936371d"
...
git-tree-sha1 = "642cdd8203d2c6669f59a4a21369e2cea9825e97"
...
version = "0.9.20"
weakdeps = ["BoundaryValueDiffEq", "Colors", "DiffEqCallbacks", "NLsolve", "OrdinaryDiffEq", "RecipesBase", "Test"]
