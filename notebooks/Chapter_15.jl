...
# ╔═╡ b98909a2-c87a-4e44-97de-2d74f1ae02c8
begin
  using Pkg, DrWatson
  using PlutoUI
  TableOfContents()
...
# ╔═╡ f8cafe39-a9b1-4fa5-833f-3e89ee872045
begin
	using Turing
	using Turing
	using DataFrames
...
md"# Chap 15: Missing Data and Other Opportunities"

# ╔═╡ c7f011df-1a78-4bef-be46-12b34c441138
versioninfo()

# ╔═╡ 33558509-3c11-46a1-9cba-a8661a0cd102
html"""
<style>
	main {
		margin: 0 auto;
...
# ╔═╡ bbe21a60-677d-4a24-a781-4ac52a2bb188
md"## Code 15.1"

# ╔═╡ 822227e3-b042-42f2-912e-4e6e1f1614a8
begin
	Random.seed!(2)
	...
	@time pancakes = vcat([sim_pancake() for _ in 1:100_000]'...)
	up = pancakes[:,1]
	down = pancakes[:,2]
...
# ╔═╡ 6c71b4ab-5e06-4054-9907-88aab4d9a832
pancake = [[1, 1], [1, 0], [0, 0]]

# ╔═╡ 94758bb7-1154-412b-9e4f-450f026c6f84
sides = sample(pancake)

# ╔═╡ bbb43f62-21ae-430d-83aa-b05730e2ec79
sample([sides, reverse(sides)])

# ╔═╡ 2566faa3-8fe8-4f3f-a832-541604afcf34
[sides, reverse(sides)]

# ╔═╡ 640769b9-8f08-44fb-a418-f8291fec221f
md" # 15.1 Measurement error"

# ╔═╡ b5c5931c-9941-450f-9039-43f1ff06c280
md"## Code 15.2"

# ╔═╡ cd34342c-aa41-42c0-9788-8149405b7c96
begin
	d_divorce = DataFrame(CSV.File("data/WaffleDivorce.csv"))
	
	scatter(d_divorce.MedianAgeMarriage, d_divorce.Divorce, 
		xlab="Median age marriage", ylab="Divorse rate")
	scatter!(d_divorce.MedianAgeMarriage, d_divorce.Divorce, yerror=d_divorce."Divorce SE", ms=0)
end

# ╔═╡ e9ab00c4-e153-4e71-9aa0-c651a0f70bf0
first(d_divorce,3)

# ╔═╡ 42e0601d-74c5-4e11-9491-f2998c598880
begin	
	scatter(d_divorce.Population, d_divorce.Divorce, xaxis=:log10,
 		xlab="Population", ylab="Divorce rate",
		xminorticks=9, yminorticks=10,
		yerror=d_divorce."Divorce SE", ms=5)
	#scatter!(d_divorce.Population, d_divorce.Divorce, yerror=d_divorce."Divorce SE", ms=0)
end

# ╔═╡ a416c1db-6c9f-48e0-ada8-11e7652a124e
md"## Code 15.3 model `m15_1`"

# ╔═╡ fa3d4e10-7eeb-46d8-aa5f-0ea042fd9701
md"## Code 15.4"

# ╔═╡ 1f23a5a0-a548-47ab-b1af-36b7ca9c7c67
md"## Code 15.5 model `m15_2`"

# ╔═╡ 8344366c-621a-4607-82fd-eeaca138b9a6
begin
	dlist2 = (
	    D_obs = standardize(ZScoreTransform, d_divorce.Divorce),
	    D_sd = d_divorce."Divorce SE" ./ std(d_divorce.Divorce),
	    M_obs = standardize(ZScoreTransform, d_divorce.Marriage),
	    M_sd = d_divorce."Marriage SE" ./ std(d_divorce.Marriage),
	    A = standardize(ZScoreTransform, d_divorce.MedianAgeMarriage),
	    N = nrow(d_divorce),
	)
end

# ╔═╡ 178dc3cf-03c3-4eaf-9edc-0a731ddd99a8
begin
	...
	Random.seed!(1)
	@time m15_2_ch = sample(m15_2(dlist2...), NUTS(), 1000)
	m15_2_df = DataFrame(m15_2_ch);
	D_true = [mean(m15_2_df[!, "D_true[$i]"]) for i ∈ 1:dlist2.N]
	M_true = [mean(m15_2_df[!, "M_true[$i]"]) for i ∈ 1:dlist2.N]
end

# ╔═╡ 29ca26c5-adff-4f49-955c-c0bd9943fa84
begin
	d_divorce_ls = (
	    D_obs = standardize(ZScoreTransform, d_divorce.Divorce),
	    D_sd = d_divorce."Divorce SE" ./ std(d_divorce.Divorce),
	    M = standardize(ZScoreTransform, d_divorce.Marriage),
	    A = standardize(ZScoreTransform, d_divorce.MedianAgeMarriage),
	    N = nrow(d_divorce),
	)
	...
	Random.seed!(1)
	@time m15_1_ch = sample(m15_1(d_divorce_ls...), NUTS(), 1000)
	m15_1_df = DataFrame(m15_1_ch);
end

# ╔═╡ ba6392c6-a08b-4315-820c-a5efd6caa07f
describe(m15_1_df)

# ╔═╡ c3e24a75-513f-4be9-8287-b48a98782738
describe(m15_2_df)

# ╔═╡ 181c0898-d3fb-4e16-b906-031a22fd7c27
md"## Figure 15.2"
...
begin
	...
	for i ∈ 1:dlist2.N
	    plot!([dlist2.A[i], dlist2.A[i]], [dlist2.D_obs[i], D_true[i]], c=:red, legend=false)
	end
	x = -2.5:0.2:3
...
# ╔═╡ 85197fa6-3538-4f45-a91c-84d9447d2727
...
begin
	...
	for i ∈ 1:dlist2.N
	    plot!([dlist2.M_obs[i], M_true[i]], [dlist2.D_obs[i], D_true[i]], c=:blue, legend=false)
	end
	x2 = -2:0.2:3
...
begin
	p3 = scatter(dlist2.M_obs, dlist2.D_obs, mc=:blue, xerror=dlist2.M_sd, yerror=dlist2.D_sd, 
	    label="observed", xlab="marriage rate (std)", ylab="divorce rate (std)")
	scatter!(M_true, D_true, mc=:white, label="true")
	
	for i ∈ 1:dlist2.N
	    plot!([dlist2.M_obs[i], M_true[i]], [dlist2.D_obs[i], D_true[i]], c=:red, legend=false)
	end
	p3
end
