...
# ╔═╡ 4dac3d4a-3d27-44f9-af33-099950952bfa
begin
  using PlutoUI
  TableOfContents()
end
...
# ╔═╡ 64998772-5a08-413c-adb8-578538eb13d9
md"# Chap 7: for m7_1, compare NUTS, MAP, Prior, NUTS given MAP"

# ╔═╡ 6ed849aa-5b70-464f-9340-5579742d5c33
versioninfo()
...
# ╔═╡ 15a81a9a-3747-40b4-b78e-78e981e378b8
md"## Construct the dataset"
...
# ╔═╡ 25fba26a-b7f8-4c0c-96b7-ad8172260d61
md"## m7_1 Definition"
...
# ╔═╡ 03ad3f6f-4724-49d2-b0e4-b7c849b1d837
md"## NUTS sampling"

# ╔═╡ 3eeeceba-9016-4aae-a9e5-8505bb898078
begin
	m7_1_NUTS_ch = sample(model_m7_1(d.mass_std, d.brain_std), NUTS(), 1000)
	m7_1_NUTS = DataFrame(m7_1_NUTS_ch)
	describe(m7_1_NUTS)
end

# ╔═╡ 6f5d9057-fb50-4100-9a1e-91c756dff4aa
std.(eachcol(m7_1_NUTS))

# ╔═╡ 6f22b86b-e06d-45fc-bcae-270bc53ce178
plot(m7_1_NUTS_ch)

# ╔═╡ 1629c4c3-1388-48eb-8229-a2f47607383b
md"## MAP estimates
- log_σ is a bit different from NUTS estimate."

# ╔═╡ 7c55e830-8f3e-4840-84da-1226bb1972a8
m7_1_MAP = optimize(model_m7_1(d.mass_std, d.brain_std), MAP())

# ╔═╡ 153fe1db-0eb7-408c-bb8a-dc9ae97a8be7
begin
	md"## `A Priori` sampling given the initial MAP estimates.
- Sampling from Prior() completely disregards the data and the estimates are essentially sampled directly from the prior distributions.
- log_σ centers around 0 with std=1.
- b center around -0.34 with std=9.8."
end

# ╔═╡ 6f8bf6b9-ad7a-44cc-9922-1d8f30b53b8a
begin
	@time m7_1_prior_ch = sample(model_m7_1(d.mass_std, d.brain_std), Prior(),
		1_000, init_theta = m7_1_MAP.values.array)
	m7_1_prior = DataFrame(m7_1_prior_ch)
	describe(m7_1_prior)
end

# ╔═╡ 025023cb-883a-4679-a11d-167ab1290620
std.(eachcol(m7_1_prior))

# ╔═╡ eda8e448-c699-4fe1-9d21-2bf1b19dbe70
plot(m7_1_prior_ch)

# ╔═╡ 91a98872-1e9c-4cb1-841f-fe25f72c28b5
md"## NUTS sampling given the initial MAP estimates"

# ╔═╡ 2a4e834a-6f76-4f62-bb37-9071f671c39b
begin
	@time m7_1_NUTS_init_ch = sample(model_m7_1(d.mass_std, d.brain_std), NUTS(),
		1_000, init_theta = m7_1_MAP.values.array)
	m7_1_NUTS_init = DataFrame(m7_1_NUTS_init_ch)
	describe(m7_1_NUTS_init)
end

# ╔═╡ 993d55d1-526e-4a37-9112-93cfaaa63bbb
std.(eachcol(m7_1_NUTS_init))

# ╔═╡ 9c15644a-a832-4fca-9d4d-dda042e2808a
plot(m7_1_NUTS_init_ch)
...
manifest_format = "2.0"
project_hash = "7251f3c4772400d2b740e49127c9ceaff06f1017"
